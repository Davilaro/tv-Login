...
class HomeController extends StateNotifier<HomeState> {
  ...
  //Funcion para recuperar las peliculas y series de la api y se llame en el momento que el
  //home sea renderizado.
  Future<void> init() async {
    await loadMoviesAndSeries();
    await loadPerformers();
  }

  void onTimeWindow(TimeWindow timeWindow) {
  ...
  Future<void> loadPerformers({
    ...
    state = result.when(
      left: (_) => state.copyWith(
        performes: const PerformersState.failed(),
      ),
     right: (list) => state.copyWith(
      performes: PerformersState.loaded(list),
     )
    );
  }
}
